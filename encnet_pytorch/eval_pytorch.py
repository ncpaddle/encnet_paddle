    ...
    for i, (image, dst) in enumerate(tbar):
        if args.eval:
            with torch.no_grad():
                ...
                # print("dst:", dst)
                # print("pred:", predicts)
                # print("dst:", dst[0].size())
                # print("pre:", predicts[0].size())
                # ###############贾再哲########
                # dst = [torch.tensor(np.random.randint(0, 18,  [1024, 2048]))]
                # predicts = [torch.tensor(np.random.randint(0, 18,  [1, 1, 1024, 2048]))]
