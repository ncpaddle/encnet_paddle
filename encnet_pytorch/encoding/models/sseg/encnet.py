...
__all__ = ['EncNet', 'EncModule', 'get_encnet', 'get_encnet_resnet50_pcontext', 'get_encnet_resnet101_citys',
           'get_encnet_resnet101_pcontext', 'get_encnet_resnet50_ade', 'get_encnet_resnet50d_citys',
           'get_encnet_resnet101_ade', 'get_encnet_resnet101_coco', 'get_encnet_resnet101d_citys']

class EncNet(BaseNet):
    def __init__(self, nclass, backbone, aux=False, se_loss=True, lateral=False,
    ...
    def forward(self, x):
        ...
        x[0] = interpolate(x[0], imsize, mode='bilinear', align_corners=False)
        if self.aux:
            auxout = self.auxlayer(features[2])
            auxout = interpolate(auxout, imsize, mode='bilinear', align_corners=False)
            x.append(auxout)
...
class EncHead(nn.Module):
    def __init__(self, in_channels, out_channels, se_loss=True, lateral=True,
        ...
        self.conv6 = nn.Sequential(nn.Dropout(p=0),  #(0.1, False)
...
def get_encnet_resnet50d_citys(pretrained=False, root='~/.encoding/models', **kwargs):
    r"""EncNet model from the paper `"Context Encoding for Semantic Segmentation"
    <https://arxiv.org/pdf/1803.08904.pdf>`_
    ...
    Examples
    --------
    >>> model = get_encnet_resnet50_city(pretrained=True)
    >>> print(model)
    """
    return get_encnet('citys', 'resnet50d', pretrained, root=root, aux=True,
                      base_size=520, crop_size=480, **kwargs)

def get_encnet_resnet101d_citys(pretrained=False, root='~/.encoding/models', **kwargs):
    r"""EncNet model from the paper `"Context Encoding for Semantic Segmentation"
    <https://arxiv.org/pdf/1803.08904.pdf>`_
    ...
    Examples
    --------
    >>> model = get_encnet_resnet101_city(pretrained=True)
    >>> print(model)
    """
    return get_encnet('citys', 'resnet101d', pretrained, root=root, aux=True, lateral=True,
                      base_size=520, crop_size=480, **kwargs)

def get_encnet_resnet101_citys(pretrained=False, root='~/.encoding/models', **kwargs):
    r"""EncNet model from the paper `"Context Encoding for Semantic Segmentation"
    <https://arxiv.org/pdf/1803.08904.pdf>`_
    ...
    >>> print(model)
    """
    return get_encnet('citys', 'resnet101', pretrained, root=root, aux=False, lateral=False,
                      base_size=520, crop_size=480, **kwargs)
...
# model = get_encnet_resnet50_ade(pretrained=False).cuda()
# print("------1----------")
